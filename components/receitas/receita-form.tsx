...
import {
  Receita, ReceitaIngrediente, CategoriaReceita, UnidadeRendimento,
  calcularCustoReceita, custoLiquido, MateriaPrima,
} from "@/types";
import { IngredienteSelector, IngredienteSelecionado } from "./ingrediente-selector";
...
import { Plus, Trash2, ArrowLeft, AlertCircle, BookOpen, Package, Sparkles, Loader2, Calculator } from "lucide-react";
...
const categorias: { value: CategoriaReceita; label: string }[] = [
  ...
  { value: "ingrediente", label: "Ingrediente / Preparo Base" },
...
function nomeIngrediente(
  ing: ReceitaIngrediente,
  mpMap: Map<string, MateriaPrima>,
  receitaMap: Map<string, Receita>
): string {
...
function unidadeIngrediente(
  ing: ReceitaIngrediente,
  mpMap: Map<string, MateriaPrima>,
  receitaMap: Map<string, Receita>
): string {
  if (ing.tipo === "receita" && ing.receitaId) {
    const r = receitaMap.get(ing.receitaId);
    return r?.unidadeRendimento ?? "—";
  }
  if (ing.materiaPrimaId) return mpMap.get(ing.materiaPrimaId)?.unidade ?? "—";
  return "—";
}

function custoIngrediente(
  ing: ReceitaIngrediente,
  mpMap: Map<string, MateriaPrima>,
  receitaMap: Map<string, Receita>,
  mps: MateriaPrima[],
  ...
  if (ing.tipo === "receita" && ing.receitaId) {
    ...
    if (!sub) return 0;
    const { custoPorcao } = calcularCustoReceita(sub, mps, todasReceitas);
    return ing.quantidade * custoPorcao;
  }
...
interface Props {
  ...
  todasReceitas: Receita[];
  initial?: Receita;
  onSave: (data: Omit<Receita, "id" | "criadoEm" | "atualizadoEm">) => void;
}

export function ReceitaForm({ materiasPrimas, todasReceitas, initial, onSave }: Props) {
  const router = useRouter();
  const { metaCmv: CMV_META } = useConfiguracaoStore();

  const [nome, setNome] = useState(initial?.nome ?? "");
  const [categoria, setCategoria] = useState<CategoriaReceita>(initial?.categoria ?? "prato_principal");
  ...
  // receitas disponíveis como sub-receita = todas exceto a atual
  const receitasParaSelector = todasReceitas.filter((r) => r.id !== initial?.id);

  const receitaRascunho: Receita = {
    id: initial?.id ?? "__rascunho__",
    nome, categoria,
    rendimento: parseFloat(rendimento) || 1,
    unidadeRendimento, ingredientes,
    precoVenda: parseFloat(precoVenda) || undefined,
    criadoEm: "", atualizadoEm: "",
  };

  const custos = calcularCustoReceita(receitaRascunho, materiasPrimas, todasReceitas);
  ...
  async function melhorarTexto() {
    if (!modoPreparo.trim() || melhorandoTexto) return;
    setMelhorandoTexto(true);
    try {
      const res = await fetch("/api/melhorar-preparo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ texto: modoPreparo }),
      });
      const data = await res.json();
      if (data.texto) setModoPreparo(data.texto);
    } finally {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Button variant="ghost" size="icon" onClick={() => router.push("/receitas")}>
          <ArrowLeft className="w-4 h-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold">{initial ? "Editar Receita" : "Nova Receita"}</h1>
          <p className="text-muted-foreground text-sm mt-0.5">Ficha técnica com cálculo automático de custo e CMV</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input placeholder="Ex: Coxinha de Frango" value={nome} onChange={(e) => setNome(e.target.value)} />
                {errors.nome && <p className="text-xs text-destructive">{errors.nome}</p>}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Categoria</Label>
                  <Select value={categoria} onValueChange={(v) => setCategoria(v as CategoriaReceita)}>
                ...
                <div className="space-y-1.5">
                  <Label>Tempo de Preparo <span className="text-muted-foreground text-xs">(min)</span></Label>
                  <Input type="number" min="1" placeholder="Ex: 30" value={tempoPreparo} onChange={(e) => setTempoPreparo(e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label>Rendimento *</Label>
                  <Input type="number" min="0.01" step="0.01" placeholder="Ex: 10" value={rendimento} onChange={(e) => setRendimento(e.target.value)} />
              ...
              <div className="space-y-1.5">
                <Label>Preço de Venda (R$) <span className="text-muted-foreground text-xs">(opcional)</span></Label>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Ingredientes & Preparos</CardTitle>
              <p className="text-xs text-muted-foreground mt-0.5">
                Adicione matérias-primas <Package className="inline w-3 h-3 text-blue-500" /> ou sub-receitas <BookOpen className="inline w-3 h-3 text-purple-500" />
              </p>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2 items-end">
                <div className="flex-1">
                  <Label className="text-xs mb-1 block">Ingrediente ou preparo</Label>
                  <IngredienteSelector
                    materiasPrimas={materiasPrimas}
                    receitas={receitasParaSelector}
                    receitaAtualId={initial?.id}
                    value={newSel}
                    onChange={setNewSel}
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">
                  Modo de Preparo <span className="text-muted-foreground font-normal text-sm">(opcional)</span>
                </CardTitle>
                <Button
                  type="button"
                  variant="outline"
                  ...
                  disabled={!modoPreparo.trim() || melhorandoTexto}
                >
                  {melhorandoTexto
            ...
            <CardContent>
              <Textarea placeholder="Descreva o passo a passo…" rows={5} value={modoPreparo} onChange={(e) => setModoPreparo(e.target.value)} />
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-4">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2 text-sm">
                ...
                <div className="border-t pt-2 flex justify-between">
                  <span className="text-muted-foreground">Custo / {unidadeRendimento === "porcoes" ? "porção" : unidadeRendimento}</span>
              ...
              {precoVenda && parseFloat(precoVenda) > 0 ? (
                <div className="border rounded-lg p-3 text-center">
                  ...
                  <p className={`text-3xl font-bold tabular-nums ${custos.cmv !== null ? cmvColor(custos.cmv, CMV_META) : ""}`}>
                    {custos.cmv !== null ? `${custos.cmv.toFixed(1)}%` : "—"}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">meta ≤ {CMV_META}%</p>
                  {custos.cmv !== null && custos.cmv > CMV_META && (
                    ...
                    return (
                      <div key={idx}>
                        ...
                        <div className="h-1.5 bg-muted rounded-full mt-0.5">
                          <div
                            className={`h-full rounded-full ${isReceita ? "bg-purple-400" : "bg-primary"}`}
          ...
          <Button variant="outline" className="w-full" onClick={() => router.push("/receitas")}>
            Cancelar
          </Button>
        </div>
      </div>
